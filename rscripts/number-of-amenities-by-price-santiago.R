...
result <- dbSendQuery(conn, "SELECT id, price from santiago")
...
for (listing_id in df$id) {
    query <- paste("SELECT COUNT(*) FROM listing_amenities_santiago WHERE id_listing = ", listing_id)
...
df$price <- df$price * CLP_TO_USD_RATE
