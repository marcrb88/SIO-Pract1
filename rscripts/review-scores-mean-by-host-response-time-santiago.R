...
result <- dbSendQuery(conn, "SELECT host_id, host_response_time FROM hosts WHERE host_id IN (SELECT host_id FROM santiago)")
...
for (h_id in final_df$host_id) {
    query1 <- paste("SELECT AVG(r.review_scores_rating) FROM santiago e, reviews_santiago r WHERE e.host_id = ", h_id)
...
ggsave("review_scores_mean_by_host_response_time_santiago.png", width = 7, height = 5, dpi = 300)
